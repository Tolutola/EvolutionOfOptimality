...
template<class T>
vector<T> maxATP_PF(const MSKint32t numvar, const MSKint32t numcon, vector<double> c1, vector<MSKint32t> aptrb1,vector<MSKint32t> aptre1,
		vector<MSKint32t> asub1, vector<double> aval1, vector<MSKboundkeye> bkc1, vector<double> blc1, vector<double> buc1,
		vector<MSKboundkeye> bkx1, vector<double> blx1, vector<T> bux2,T atpFlux) {
		
	// the same formulation of the optimization problem was used as the maximum biomass production because the computations are cheaper
	//and analysis in matlab shows the results are the same
